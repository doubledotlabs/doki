...
internal interface AnkoContext<out T> : ViewManager {
    val ctx: Context
    val owner: T
    val view: View
...
internal open class AnkoContextImpl<T>(
    override val ctx: Context,
    override val owner: T,
...
internal inline fun Context.UI(setContentView: Boolean, init: AnkoContext<Context>.() -> Unit): AnkoContext<Context> =
    createAnkoContext(this, init, setContentView)

internal inline fun Context.UI(init: AnkoContext<Context>.() -> Unit): AnkoContext<Context> =
    createAnkoContext(this, init)

internal inline fun Fragment.UI(init: AnkoContext<Fragment>.() -> Unit): AnkoContext<Fragment>? =
    context?.let { createAnkoContext(it, init) } ?: activity?.let { createAnkoContext(it, init) }

internal interface AnkoComponent<in T> {
    fun createView(ui: AnkoContext<T>): View
}

internal fun <T : Activity> AnkoComponent<T>.setContentView(activity: T): View =
